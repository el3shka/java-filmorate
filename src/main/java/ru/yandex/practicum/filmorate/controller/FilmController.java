...
public class FilmController {
    ...
    public static void validateFilm(Film film)
    {
        if (film.getName() == null || film.getName().isBlank())
        {
            ...
            log.info("В фильме с id = {} превышение длины описания", film.getId());
            ...
            if (film.getReleaseDate() == null || film.getReleaseDate().isBefore(THE_FIRST_FILM_REALISE_DATE)) {
                log.info("В фильме с id = {} дата релиза некорректна", film.getId());
                throw new ValidationException("Дата релиза некорректная.");
            } else if (film.getDuration() <= 0) {
                log.info("В фильме с id = {} продолжительность указана некорректно", film.getId());
